...
mvp一般最通常被用在企业级应用程序中，尽可能多的服用展现逻辑在其中是很必要的。非常复杂的视图的应用程序和大量用户交互可能发现这里mvc不太适合这个需求作为需要重度依赖多控制器的问题的解决方案。在mvp中，所有复杂逻辑都能够被封装到展现者中，它能够更好简化维护。
...
backbone贡献者赞同这种思维方式，当她将视图分离成各自不同的组成部分时，她需要一些东西来把它们组装起来。
